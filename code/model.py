...
class Encoder(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        return torch.relu(self.conv2(x1, edge_index, edge_attr))
...
class CCGNN(nn.Module):  # noqa
    def __init__(self, data_dict, device, embedding_size=128):
        ...
        self.r_mi = CLUBSample(embedding_size, embedding_size, embedding_size)
        self.p_mi = CLUBSample(embedding_size, embedding_size, embedding_size)
    ...
    def forward(self, samples, labels):
        ...
        mim_loss = self.r_mi(rr_hr, rp_hr)[1] + self.p_mi(pp_hp, rp_hp)[1]
        loss = pred_loss + self.alpha * (pred_i_loss + pred_c_loss) + self.gamma * mim_loss
        return out, loss
